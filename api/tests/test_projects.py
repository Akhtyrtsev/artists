...
class ProjectTest(APITestCase):
    ...
    def perform_auth(self, user="client"):
        ...
        if user == "client":
            basic_auth_header = (
                "Basic "
                + base64.b64encode(
                    f"{self.client_user.username}:pass12345".encode()
                ).decode()
            )
        elif user == "artist":
            basic_auth_header = (
                "Basic "
                + base64.b64encode(
                    f"{self.artist.username}:pass12345".encode()
                ).decode()
            )
    ...
    def test_post_client_media(self):
        ...
        data = {"name": "name", "project": self.project.pk, "file": image}
        response = self.client.post(url, data=data)
        self.assertEquals(response.status_code, 201)
        self.assertEquals(
            Project.objects.filter(client=self.client_user).last().media.all().count(),
            2,
        )
        self.assertEquals(
            Project.objects.filter(client=self.client_user)
            .last()
            .media.all()
            .last()
            .name,
