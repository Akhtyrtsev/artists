...
class ProjectAdmin(admin.ModelAdmin):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "client":
            ...
            kwargs["queryset"] = Group.objects.filter(name__icontains="client").last().user_set.all()

        if db_field.name == "artist":
            # Define the condition for filtering the choices
            kwargs["queryset"] = Group.objects.filter(name__icontains="artist").last().user_set.all()
